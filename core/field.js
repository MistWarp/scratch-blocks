...
Blockly.Field.getCachedWidth = function(textElement) {
  ...
  var font = Blockly.Fonts.getFont(textElement.className.baseVal);
  return Blockly.PretextMeasurement.measureWidth(text, font);
};

/**
 * Clear all text measurement caches.
 */
Blockly.Field.clearFontCache = function() {
  Blockly.PretextMeasurement.clearCache();
};

/**
 * Start caching field widths. No-op - kept for backward compatibility.
 */
Blockly.Field.startCache = function() {
  // No-op - pretext handles caching internally
};

/**
 * Stop caching field widths. No-op - kept for backward compatibility.
 */
